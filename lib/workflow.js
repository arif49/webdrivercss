...
function workflow(pagename, args) {
    ...
    var baseLineBase = this.screenshotRoot + '/baseline/' + currentArgs.baseSubFolder;
    var regressionBase = this.screenshotRoot + '/target/regression/' + currentArgs.baseSubFolder;
    var diffBase = this.screenshotRoot + '/target/regression/' + currentArgs.baseSubFolder + '/diff';

    createDirectory(baseLineBase);
    createDirectory(regressionBase);
    createDirectory(diffBase);
    ...
    var context = {
        ...
        baselinePath: baseLineBase + '/' + pagename + '.' + currentArgs.name + '.baseline.png',
        regressionPath: regressionBase + '/' + pagename + '.' + currentArgs.name + '.regression.png',
        diffPath: diffBase + '/' + pagename + '.' + currentArgs.name + '.diff.png',
        screenshot: this.screenshotRoot + '/' + pagename + '.png',
        isComparable: false,
        warnings: [],
        newScreenSize: 0,
        pageInfo: null,
        updateBaseline: (typeof currentArgs.updateBaseline === 'boolean') ? currentArgs.updateBaseline : false,
